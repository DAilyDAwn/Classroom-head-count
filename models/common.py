...
class Conv(nn.Module):
    ...
    """
    default_act = nn.SiLU()  # default activation；sigmoid加权线性单元(SiLU)

    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True):
        super().__init__()
        self.conv = nn.Conv2d(c1, c2, k, s, autopad(k, p, d), groups=g, dilation=d, bias=False) # 把输入的通道分成g组，此时每一个输出通道只需要在其中一组上做卷积；dilation是对kernel进行膨胀，多出来的空隙用0填充
        self.bn = nn.BatchNorm2d(c2)
        self.act = self.default_act if act is True else act if isinstance(act, nn.Module) else nn.Identity() # nn.Identity()建立一个输入模块，什么都不做，通常用在神经网络的输入层
